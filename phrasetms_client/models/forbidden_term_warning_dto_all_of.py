...
class ForbiddenTermWarningDtoAllOf(BaseModel):
    """
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ForbiddenTermWarningDtoAllOf:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in source_terms (list)
        _items = []
        if self.source_terms:
            for _item in self.source_terms:
                if _item:
                    _items.append(_item.to_dict())
            _dict['sourceTerms'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> ForbiddenTermWarningDtoAllOf:
