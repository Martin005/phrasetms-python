...
class QACheckDtoV2(BaseModel):
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ("VOID", "NUMBER", "STRING", "REGEX", "MORAVIA"):
            raise ValueError(
                "must be one of enum values ('VOID', 'NUMBER', 'STRING', 'REGEX', 'MORAVIA')"
            )
        return value
    ...
    def name_validate_enum(cls, value):
        ...
        if value not in (
            ...
            raise ValueError(
                "must be one of enum values ('emptyTarget', 'inconsistentTranslation', 'joinMarksInconsistency', 'missingNumber', 'segmentNotConfirmed', 'nonConformingTerms', 'multipleSpaces', 'endPunctuation', 'targetLength', 'absoluteTargetLength', 'relativeTargetLength', 'inconsistentFormatting', 'unresolvedComment', 'emptyPairTags', 'strictJobStatus', 'forbiddenStringsEnabled', 'excludeLockedSegments', 'ignoreNotApprovedTerms', 'spellCheck', 'repeatedWords', 'inconsistentTagContent', 'emptyTagContent', 'malformed', 'forbiddenTerms', 'targetLengthPercent', 'targetLengthPerSegment', 'newerAtLowerLevel', 'leadingAndTrailingSpaces', 'targetSourceIdentical', 'ignoreInAllWorkflowSteps', 'regexp', 'unmodifiedFuzzyTranslation', 'unmodifiedFuzzyTranslationTM', 'unmodifiedFuzzyTranslationMTNT', 'moravia', 'extraNumbers', 'nestedTags')"
            )
        return value
    ...
    def from_json(
        cls, json_str: str
    ) -> Union["MORAVIA", "NUMBER", "REGEX", "STRING", "VOID"]:
    ...
    def from_dict(
        ...
        if object_type:
            ...
            raise ValueError(
                "QACheckDtoV2 failed to lookup discriminator value from "
                + json.dumps(obj)
