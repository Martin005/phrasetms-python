...
class SpellCheckWarningDtoAllOf(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SpellCheckWarningDtoAllOf:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in misspelled_words (list)
        _items = []
        if self.misspelled_words:
            for _item in self.misspelled_words:
                if _item:
                    _items.append(_item.to_dict())
            _dict['misspelledWords'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> SpellCheckWarningDtoAllOf:
