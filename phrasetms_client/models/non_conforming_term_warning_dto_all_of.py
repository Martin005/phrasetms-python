...
class NonConformingTermWarningDtoAllOf(BaseModel):
    """
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NonConformingTermWarningDtoAllOf:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in suggested_target_terms (list)
        _items = []
        if self.suggested_target_terms:
            for _item in self.suggested_target_terms:
                if _item:
                    _items.append(_item.to_dict())
            _dict['suggestedTargetTerms'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> NonConformingTermWarningDtoAllOf:
