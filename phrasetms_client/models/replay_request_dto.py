...
class ReplayRequestDto(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ReplayRequestDto:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in webhook_calls (list)
        _items = []
        if self.webhook_calls:
            for _item in self.webhook_calls:
                if _item:
                    _items.append(_item.to_dict())
            _dict['webhookCalls'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> ReplayRequestDto:
