...
class SpellCheckResponseDto(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SpellCheckResponseDto:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in spell_check_results (list)
        _items = []
        if self.spell_check_results:
            for _item in self.spell_check_results:
                if _item:
                    _items.append(_item.to_dict())
            _dict['spellCheckResults'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> SpellCheckResponseDto:
