...
class QuoteV2Dto(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> QuoteV2Dto:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in additional_steps (list)
        _items = []
        if self.additional_steps:
            for _item in self.additional_steps:
                if _item:
                    _items.append(_item.to_dict())
            _dict['additionalSteps'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> QuoteV2Dto:
        ...
        _obj = QuoteV2Dto.parse_obj({
            ...
            "additional_steps": [AdditionalWorkflowStepV2Dto.from_dict(_item) for _item in obj.get("additionalSteps")] if obj.get("additionalSteps") is not None else None
        })
        return _obj
